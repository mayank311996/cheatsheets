...
parser.add_argument("-i", "--input", action="store", dest="inp",
                    help="Take a sample image and classify it", type=str)
parser.add_argument("-t", "--train", action="store_true",
                    help="Run the training of the model")
parser.add_argument("-p", "--preprocess", action="store_true",
                    help="Update the train and test csv files with the new images in dataset, used this if you added new images in dataset")
...
validation_set = LocalDataset(IMAGES_PATH, VALIDATION_PATH,
                              transform=transform)

training_set_loader = DataLoader(dataset=training_set, batch_size=BATCH_SIZE,
                                 num_workers=THREADS, shuffle=True)
validation_set_loader = DataLoader(dataset=validation_set,
                                   batch_size=BATCH_SIZE, num_workers=THREADS,
                                   shuffle=False)


def train_model(model_name, model, lr=LEARNING_RATE, epochs=EPOCHS,
        ...
        for mode in ['train', 'test']:
            ...
            try:
                for i, batch in enumerate(loaders[mode]):
                    ...
                    if USE_CUDA and cuda_available:
                        acc = accuracy_score(y.data.cuda().cpu().numpy(),
                                             output.max(1)[
                                                 1].cuda().cpu().numpy())
                    else:
                    ...
                    print(
                        "\r[%s] Epoch %d/%d. Iteration %d/%d. Loss: %0.2f. Accuracy: %0.2f" % \
                        (mode, e + 1, epochs, i, len(loaders[mode]),
                         epoch_loss / samples, epoch_acc / samples))
    ...
    torch.save(model.state_dict(),
               str(RESULTS_PATH) + "/" + str(model_name) + "/" + str(
                   model_name) + ".pt")
...
def write_stats(model_name, y, predictions, gts, predictions2):
    ...
    file = open(str(RESULTS_PATH) + "/" + str(model_name) + "/" + str(
        model_name) + "_stats.txt", "w+")
...
def plot_logs_classification(model_name, logs):
    ...
    plt.grid()
    # plt.show()
    plt.savefig(str(RESULTS_PATH) + "/" + str(model_name) + "/" + str(
        model_name) + "_graph.png")


def train_model_iter(model_name, model, weight_decay=0):
    if args.train:
        model, loss_acc, y_testing, preds = train_model(model_name=model_name,
                                                        model=model,
                                                        weight_decay=weight_decay)
...
if args.inp:
    ...
    print(str(RESULTS_PATH) + "/" + str(model_name) + "/" + str(
        model_name) + ".pt")
